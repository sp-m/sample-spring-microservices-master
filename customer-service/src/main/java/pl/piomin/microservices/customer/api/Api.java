...
public class Api {
	...
	protected Logger log = Logger.getLogger(Api.class.getName());
    ...
    public Api() {
        ...
        customers.add(new Customer(1, "12345", "AdamKowalski", CustomerType.INDIVIDUAL));
        customers.add(new Customer(2, "12346", "AnnaMalinowska", CustomerType.INDIVIDUAL));
        customers.add(new Customer(3, "12347", "PawełMichalski", CustomerType.INDIVIDUAL));
        customers.add(new Customer(4, "12348", "KarolinaLewandowska", CustomerType.INDIVIDUAL));
    }

    @RequestMapping(method = RequestMethod.GET, value = "/passport/{passport}")
    public Customer findByPassport(@PathVariable("passport") String passport) throws CustomerNotFoundException {
        log.info(String.format("Customer.findByPesel(%s)", passport));
        return customers.stream()
                .filter(it -> it.getPassport().equals(passport))
                .findFirst()
                .orElseThrow(() -> new CustomerNotFoundException("passport : " + passport));
    }
